...
void makeMove(Board *boardPtr, unsigned i)
	...
	for unsigned i = 0; i < group.size; ++i
		...
		GRID(rr, cc) = 0
		col_touched[cc] = true
	// for each touched column, compact non-zero tiles downward
	for unsigned cc = 0; cc < boardPtr->C; ++cc
		if !col_touched[cc]
			continue
		// write_idx is the row index where the next non-zero tile should be written
		int write_idx = R - 1
		// scan from bottom to top, copy non-zero tiles down
